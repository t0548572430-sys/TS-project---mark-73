...
let products: Product[] = JSON.parse(localStorage.getItem('lalineProducts') || JSON.stringify([
    {
        id: 1,
...
function switchAdminTab(tabName: string): void {
    const tabs = document.querySelectorAll<HTMLElement>('.admin-tab-btn');
...
function renderStats(): void {
    ...
    if (totalProductsEl) totalProductsEl.textContent = products.length.toString();
}
...
function viewOrder(index: number): void {
    const orders: OrderData[] = JSON.parse(localStorage.getItem('lalineOrders') || '[]');
...
document.addEventListener('DOMContentLoaded', (): void => {
    ...
    // Tab switching
    document.querySelectorAll<HTMLElement>('.admin-tab-btn').forEach(tab => {
    ...
    if (addProductForm) {
        addProductForm.addEventListener('submit', (e) => {
            ...
            const oldPriceValue = formData.get('productOldPrice') as string;
            const badgeValue = formData.get('productBadge') as string;
...
// Export to make this file a module with isolated scope
export {};